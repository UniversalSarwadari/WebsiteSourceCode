...
use Barn2\Plugin\WC_Bulk_Variations\Dependencies\Barn2\Setup_Wizard\Interfaces\Deferrable;
...
class Api implements JsonSerializable
{
    ...
    public function save_fields($request)
    {
        ...
        if (empty($step) || !$step instanceof Step) {
            return self::send_error_response(['message' => __('Could not find the appropriate step.', 'woocommerce-bulk-variations')]);
        }
    ...
    public function handle_license($request)
    {
        ...
        if (empty($license_key)) {
            return self::send_error_response(['message' => __('Please enter a license key.', 'woocommerce-bulk-variations')]);
        }
        if (!\in_array($action, $allowed_actions, \true)) {
            return self::send_error_response(['message' => __('Invalid action requested.', 'woocommerce-bulk-variations')]);
        }
        $license_handler = $this->get_plugin()->get_license();
        switch ($action) {
            case 'activate':
