...
namespace Barn2\WBV_Lib\Plugin\Admin;

use Barn2\WBV_Lib\Plugin\Plugin;
use Barn2\WBV_Lib\Registerable;
use Barn2\WBV_Lib\Service;
...
class Admin_Links implements Registerable, Service {
	...
	public function add_settings_link( $links ) {
		...
		array_unshift( $links, sprintf( '<a href="%1$s">%2$s</a>', esc_url( $settings_url ), __( 'Settings', 'woocommerce-bulk-variations' ) ) );
	...
	public function add_documentation_link( $links, $file ) {
		...
		$row_meta = [
			'docs' => sprintf(
				...
				esc_attr( sprintf( __( 'View %s documentation', 'woocommerce-bulk-variations' ), $this->plugin->get_name() ) ),
				esc_html__( 'Docs', 'woocommerce-bulk-variations' )
			)
		];
